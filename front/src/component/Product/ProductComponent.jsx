...
export default function ProductComponent({ order }) {
	...
	return (
		<>
			<AccordionDetails style={{ overflow: 'visible', height: 'auto' }}>
				<ImageList
					sx={{ width: 'auto', height: 'auto', overflow: 'visible' }}
					cols={1}
					gap={10}
					rowHeight={'auto'}
				>
					<ImageListItem
						key='Subheader'
						cols={2}
						style={{ height: 'auto', marginLeft: 25 }}
					>
								...
								<div style={{ width: '100%' }}>
									{open[product.productId.name] && (
										<Alert
											...
											variant='filled'
											severity='error'
											style={{ marginTop: 10, maxWidth: 208 }}
											onClose={() =>
												setOpen((prevState) => ({
													...prevState,
										...
										>
											Product unavailable. Please select another from the
											suggestions below.
										</Alert>
									)}
								</div>
								<ImageListItem style={{ marginLeft: 10 }}>
									<Typography variant='h7' sx={{ fontFamily: 'inherit' }}>
