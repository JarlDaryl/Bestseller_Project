...
export default function GenerateProductSuggestionComponent({
	...
	const addToOrder = (productId, quantity) => {
		...
		if (productInOrder) {
			...
			setTotalPrice(
				(prevTotal) => prevTotal + selectedProduct.price * quantity
			);
			return;
		}
		...
		const existingProductInAddedList = productAddedList.find(
			(product) => product._id === productId
		);
	...
	const handleAccept = () => {
		...
		const newProducts = productAddedList.filter(
			(product) =>
				!originalProducts.some(
		...
		for (let product of viableProducts) {
			let productExists = updatedOrder.products.some(
				(orderProduct) => orderProduct.productId._id === product._id
			);
		...
		sessionStorage.setItem('updatedOrder', JSON.stringify(updatedOrder));
		handleClose();
	};
	...
	return (
		<>
			{!loading && !error && (
				<div>
					<Button onClick={handleOpen} className='similar-products-open-button'>
						Similar products
					</Button>
					<Modal open={open} onClose={handleClose}>
						<Box sx={{ ...style, overflowY: 'auto' }}>
							...
							<Box className='similar-products-buttons-container '>
								...
								<Button onClick={handleAccept} className='accept-close-button'>
									{' '}
									<CheckCircleIcon />
								</Button>
							</Box>
					...
					<Grid container direction='row' spacing={2}>
						<NewSuggestedProductsAddedComponent
							productAddedList={productAddedList}
							quantity={quantity}
							isAddToOrderClicked={isAddToOrderClicked}
						/>
					</Grid>
				</div>
			)}
